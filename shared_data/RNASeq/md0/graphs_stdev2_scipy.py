...
cmd.reinitialize()
files = [
    ('4f3t.cif', f'step5_{i * 10 + 1}.gro'),
    ('4f3t.cif', f'step5_{i * 10 + 2}.gro'),
    ('4f3t.cif', f'step5_{i * 10 + 3}.gro'),
    ('4f3t.cif', f'step5_{i * 10 + 4}.gro'),
    ('4f3t.cif', f'step5_{i * 10 + 5}.gro'),
    ('4f3t.cif', f'step5_{i * 10 + 6}.gro'),
    ('4f3t.cif', f'step5_{i * 10 + 7}.gro'),
    ('4f3t.cif', f'step5_{i * 10 + 8}.gro'),
    ('4f3t.cif', f'step5_{i * 10 + 9}.gro'),
    ('4f3t.cif', f'step5_{i * 10 + 10}.gro'),
...
combined_index_A = list(range(22, 200))  #22 200
...
import numpy as np
from scipy.optimize import minimize
def mse_curve(params, x, y):
    curve = np.polyval(params, x)
    mse = np.mean((y - curve) ** 2)
    return mse
all_distances = []
for distances in combined_distances_A.values():
    all_distances.extend(distances)
combined_distances = np.array(all_distances)
x = np.linspace(min(combined_index_A), max(combined_index_A), len(combined_distances))
initial_params = np.zeros(4)
result = minimize(mse_curve, initial_params, args=(x, combined_distances), method='BFGS')
optimized_params = result.x
y_optimized = np.polyval(optimized_params, x)
plt.figure(figsize=(10, 6))
for distances in combined_distances_A.values():
    plt.plot(combined_index_A[:len(distances)], distances, color='gray', alpha=0.5)
plt.plot(x, y_optimized, color='red', linewidth=2, label='Optimized Curve (Minimized MSE)')
plt.title('Optimized Curve (Minimized MSE)')
plt.xlabel('x')
plt.ylabel('y')
plt.legend()
plt.show()
